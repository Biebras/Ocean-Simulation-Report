...
%         \hline
%         $k_{S_1}$ & Subsurface Scattering Intensity \\
%         \hline
%         $k_{S_2}$ & Subsurface Scattering Intensity \\
%         \hline
%     \end{tabular}
...
% \begin{equation}
%     \begin{split}
%         L_S &= (L_{S_1} + L_{S_2}) C_{WS} C_L\\
%         L_{S_1} &= k_{S_1} \max(0, H) ([D_S, -D_V])^{4}(0.5-0.5(D_s \cdot N))^{3}\\
%         L_{S_2} &= k_{S_2} ([D_V, N])^{2}
%     \end{split}
% \end{equation}
...
\begin{minipage}{1\textwidth}
    ...
    \captionof{figure}{Ocean with Tilling}
...
Another approuch is to simulate multiple cascades for diffrent wave lengths $k$ \ref{fig:cascades}. We can split our simulation into 3 parts, big waves, medium waves and small waves.
...
\begin{minipage}{1\textwidth}
    ...
    \captionof{figure}{Ocean with Cascades}
